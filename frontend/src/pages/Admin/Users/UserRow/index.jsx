...
export default function UserRow({ currUser, user }) {
  ...
  const handleSuspend = async () => {
    if (
      !window.confirm(
        `Are you sure you want to suspend ${user.username}?\nAfter you do this they will be logged out and unable to log back into this instance of Orion until unsuspended by an admin.`
      )
    )
      return false;
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `Are you sure you want to delete ${user.username}?\nAfter you do this they will be logged out and unable to use this instance of Orion.\n\nThis action is irreversible.`
      )
    )
      return false;
          ...
          {currUser?.id !== user.id && canModify && (
            <>
              <button
                ...
                className="text-xs font-medium text-white/80 light:text-black/80 light:hover:text-orange-500 hover:text-orange-300 rounded-lg px-2 py-1 hover:bg-white light:hover:bg-orange-50 hover:bg-opacity-10"
              >
