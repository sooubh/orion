...
const markdown = markdownIt({
  ...
  highlight: function (code, lang) {
    ...
    if (lang && hljs.getLanguage(lang)) {
      try {
        return (
          `<div class="whitespace-pre-line w-full max-w-[65vw] hljs ${theme} border border-slate-200 dark:border-[#1f2328] rounded-xl relative font-mono font-normal text-sm overflow-hidden">
            <div class="w-full flex items-center sticky top-0 text-slate-700 dark:text-zinc-200 bg-slate-100 dark:bg-stone-800 border-b border-slate-200 dark:border-stone-700 px-4 py-2 text-xs font-sans justify-between rounded-t-xl -mt-5">
              <div class="flex gap-2">
                <code class="text-xs">${lang || ""}</code>
              </div>
              <button data-code-snippet data-code="code-${uuid}" class="flex items-center gap-x-1 cursor-pointer">
...
markdown.renderer.rules.strong_open = () => '<strong class="text-slate-900 dark:text-white font-semibold">';
